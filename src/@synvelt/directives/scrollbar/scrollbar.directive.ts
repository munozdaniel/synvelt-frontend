...
import {
    ScrollbarGeometry,
    ScrollbarPosition,
} from '@synvelt/directives/scrollbar/scrollbar.types';

/**
 * Wrapper directive for the Perfect Scrollbar: https://github.com/mdbootstrap/perfect-scrollbar
 */
@Directive({
    selector: '[synveltScrollbar]',
    exportAs: 'synveltScrollbar',
})
export class SynveltScrollbarDirective implements OnChanges, OnInit, OnDestroy {
    /* eslint-disable @typescript-eslint/naming-convention */
    static ngAcceptInputType_synveltScrollbar: BooleanInput;
    /* eslint-enable @typescript-eslint/naming-convention */

    @Input() synveltScrollbar: boolean = true;
    @Input() synveltScrollbarOptions: PerfectScrollbar.Options;
    ...
    ngOnChanges(changes: SimpleChanges): void {
        // Enabled
        if ('synveltScrollbar' in changes) {
            // Interpret empty string as 'true'
            this.synveltScrollbar = coerceBooleanProperty(
                changes.synveltScrollbar.currentValue
            );

            // If enabled, init the directive
            if (this.synveltScrollbar) {
                this._init();
            }
        ...
        // Scrollbar options
        if ('synveltScrollbarOptions' in changes) {
            // Merge the options
            this._options = merge(
                {},
                this._options,
                changes.synveltScrollbarOptions.currentValue
            );
