...
import {
    SynveltNavigationItem,
    SynveltVerticalNavigationAppearance,
    SynveltVerticalNavigationMode,
    SynveltVerticalNavigationPosition,
} from '@synvelt/components/navigation/navigation.types';
...
@Component({
    selector: 'synvelt-vertical-navigation',
    templateUrl: './vertical.component.html',
    styleUrls: ['./vertical.component.scss'],
    animations: synveltAnimations,
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    exportAs: 'synveltVerticalNavigation',
})
export class SynveltVerticalNavigationComponent
    implements OnChanges, OnInit, AfterViewInit, OnDestroy
{
    ...
    @Input() appearance: SynveltVerticalNavigationAppearance = 'default';
    @Input() autoCollapse: boolean = true;
    @Input() inner: boolean = false;
    @Input() mode: SynveltVerticalNavigationMode = 'side';
    @Input() name: string = this._synveltUtilsService.randomId();
    @Input() navigation: SynveltNavigationItem[];
    @Input() opened: boolean = true;
    @Input() position: SynveltVerticalNavigationPosition = 'left';
    @Input() transparentOverlay: boolean = false;
    @Output()
    readonly appearanceChanged: EventEmitter<SynveltVerticalNavigationAppearance> =
        new EventEmitter<SynveltVerticalNavigationAppearance>();
    @Output()
    readonly modeChanged: EventEmitter<SynveltVerticalNavigationMode> = new EventEmitter<SynveltVerticalNavigationMode>();
    @Output() readonly openedChanged: EventEmitter<boolean> =
        new EventEmitter<boolean>();
    @Output()
    readonly positionChanged: EventEmitter<SynveltVerticalNavigationPosition> =
        new EventEmitter<SynveltVerticalNavigationPosition>();
    @ViewChild('navigationContent') private _navigationContentEl: ElementRef;

    activeAsideItemId: string | null = null;
    onCollapsableItemCollapsed: ReplaySubject<SynveltNavigationItem> =
        new ReplaySubject<SynveltNavigationItem>(1);
    onCollapsableItemExpanded: ReplaySubject<SynveltNavigationItem> =
        new ReplaySubject<SynveltNavigationItem>(1);
    ...
    private _synveltScrollbarDirectives!: QueryList<SynveltScrollbarDirective>;
    private _synveltScrollbarDirectivesSubscription: Subscription;
    ...
    @HostBinding('class') get classList(): any {
        return {
            ...
            [`synvelt-vertical-navigation-appearance-${this.appearance}`]: true,
            'synvelt-vertical-navigation-hover': this._hovered,
            'synvelt-vertical-navigation-inner': this.inner,
            'synvelt-vertical-navigation-mode-over': this.mode === 'over',
            'synvelt-vertical-navigation-mode-side': this.mode === 'side',
            'synvelt-vertical-navigation-opened': this.opened,
    ...
    /**
     * Setter for synveltScrollbarDirectives
     */
    @ViewChildren(SynveltScrollbarDirective)
    set synveltScrollbarDirectives(
        synveltScrollbarDirectives: QueryList<SynveltScrollbarDirective>
    ) {
        // Store the directives
        this._synveltScrollbarDirectives = synveltScrollbarDirectives;

        // Return if there are no directives
        if (synveltScrollbarDirectives.length === 0) {
            return;
        }

        // Unsubscribe the previous subscriptions
        if (this._synveltScrollbarDirectivesSubscription) {
            this._synveltScrollbarDirectivesSubscription.unsubscribe();
        }

        // Update the scrollbars on collapsable items' collapse/expand
        this._synveltScrollbarDirectivesSubscription = merge(
            ...
            .subscribe(() => {
                ...
                synveltScrollbarDirectives.forEach(
                    (synveltScrollbarDirective) => {
                        synveltScrollbarDirective.update();
                    }
                );
            });
    }
    ...
    ngAfterViewInit(): void {
        setTimeout(() => {
            ...
            // Otherwise
                ...
                this._synveltScrollbarDirectives.forEach(
                    (synveltScrollbarDirective) => {
                        // Skip if not enabled
                        if (!synveltScrollbarDirective.isEnabled()) {
                            return;
                        }

                        // Scroll to the active element
                        synveltScrollbarDirective.scrollToElement(
                            '.synvelt-vertical-navigation-item-active',
                            -120,
                            true
                        );
     ...
     * @param item
     */
    openAside(item: SynveltNavigationItem): void {
     ...
     * @param item
     */
    toggleAside(item: SynveltNavigationItem): void {
        // Toggle
    ...
    private _showOverlay(): void {
        ...
        this._overlay.classList.add('synvelt-vertical-navigation-overlay');

        // Add a class depending on the transparentOverlay option
        if (this.transparentOverlay) {
            this._overlay.classList.add(
                'synvelt-vertical-navigation-overlay-transparent'
            );
        }
