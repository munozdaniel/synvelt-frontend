...
const generateVariableColors = (theme) => {
    ...
    const customPropertiesWithOpacity =
        ...
        ({ opacityVariable, opacityValue }) => {
            if (opacityValue) {
                return `rgba(var(--synvelt-${name}-rgb), ${opacityValue})`;
            }
            if (opacityVariable) {
                return `rgba(var(--synvelt-${name}-rgb), var(${opacityVariable}, 1))`;
            }
            return `rgb(var(--synvelt-${name}-rgb))`;
        };
...
// -----------------------------------------------------------------------------------------------------
// @ SYNVELT TailwindCSS Main Plugin
// -----------------------------------------------------------------------------------------------------
            ...
            const schemeCustomProps = _.map(
                ...
                (colorScheme) => {
                    ...
                    const background = theme(
                        `synvelt.customProps.background.${colorScheme}`
                    );
                    const foreground = theme(
                        `synvelt.customProps.foreground.${colorScheme}`
                    );
