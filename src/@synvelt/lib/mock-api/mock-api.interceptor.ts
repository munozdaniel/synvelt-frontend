...
import { SynveltMockApiService } from '@synvelt/lib/mock-api/mock-api.service';

@Injectable({
    providedIn: 'root',
})
export class SynveltMockApiInterceptor implements HttpInterceptor {
    /**
     * Constructor
     */
    constructor(
        @Inject(SYNVELT_MOCK_API_DEFAULT_DELAY) private _defaultDelay: number,
        private _synveltMockApiService: SynveltMockApiService
    ) {}
    ...
    intercept(
        ...
        const { handler, urlParams } = this._synveltMockApiService.findHandler(
