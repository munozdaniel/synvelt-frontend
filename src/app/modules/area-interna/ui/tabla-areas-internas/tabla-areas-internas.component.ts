...
@Component({
  ...
  templateUrl: './tabla-areas-internas.component.html',
  animations: [
    synveltAnimations,
...
export class TablaAreasInternasComponent implements OnInit, OnChanges {
  ...
  expandedElement: IAreaInterna | null;
  // Mobile
  isMobile: boolean;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes.areasInternas && changes.areasInternas.currentValue) {
      this.dataSource.data = this.areasInternas;
    }
  }
  ...
  customSearchSortTable() {
    this.dataSource.filterPredicate = (data: IAreaInterna, filters: string) => {
      const matchFilter = [];
    ...
    this.dataSource.sortingDataAccessor = (item: IAreaInterna, property) => {
  ...
  editar(row: IAreaInterna) {
    this.retEditar.emit(row.id);
  }
  eliminar(row: IAreaInterna) {
