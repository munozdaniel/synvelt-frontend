...
export class ChoferService {
   ...
   * @returns
   */
  obtenertodos(parametros?: any): Observable<IChofer[]> {
    const queryParams = this.setQueryParams(parametros);
    return this._http.get<any>(this.url + 'operacion/ListaChofer', {
      params: queryParams,
    });
  }
  ...
  guardar(id: string, parametros: any): Observable<IChofer> {
    const headers = new HttpHeaders().append(
      'Content-Type',
      'multipart/form-data'
    );
    const queryParams = this.setQueryParams({ id });
    return this._http.post<IChofer>(
      this.url + 'operacion/ActualizacionChofer',
      { ...parametros },
      {
        headers: headers,
        params: queryParams,
  ...
  eliminar(id): Observable<void> {
    ...
    const queryParams = this.setQueryParams({ id });
    return this._http.get<void>(this.url + 'operacion/BajaChofer', {
